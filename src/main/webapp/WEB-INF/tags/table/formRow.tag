...
<button id="${name}" class="btn btn-white btn-sm" data-toggle="tooltip" data-placement="left" onclick="${name}edit()" title="修改"><i class="fa ${icon==null?'fa-pencil':icon}""></i> ${label==null?'修改':label}</button>
                        </button>
	...
	function ${name}edit(){
		  ...
		  if(size > 1 ){
		    ...
		    if('${pageModel}'==null || '${pageModel}'==''){
		    	openDialog("${label==null?'修改':label}"+'${title}',"${url}?id="+id,"${width==null?'800px':width}", "${height==null?'500px':height}","${target}");
		    }else{
