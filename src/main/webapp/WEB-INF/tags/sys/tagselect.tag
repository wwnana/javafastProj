...
$(function () {
    $.post("${url}", { "action": "one" }, function (data) {
        ...
        for (var i = 0; i < table.length; i++) {
            ...
            if('${value}'!=null && '${value}'.indexOf(item.name) >= 0){
            	
            	$("#${id}Id").append("<option selected='selected' value=" + item.name + ">" + item.name + "</option>");
            }else{
            	$("#${id}Id").append("<option value=" + item.name + ">" + item.name + "</option>");
            }          
        }
        //返回的是json格式的数据
    }, "json");
