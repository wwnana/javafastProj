...
<%@ attribute name="fileNumLimit" type="java.lang.String" required="false" description="最多上传多少文件"%>
<ol id="${input}Preview" class="ol-image-list"></ol>
<c:if test="${!readonly}"><a href="javascript:" onclick="${input}FinderOpen();" class="btn btn-info">${selectMultiple?'添加':'选择'}</a>&nbsp;<a href="javascript:" onclick="${input}DelAll();" class="btn btn-default">清除</a></c:if>
	...
	function ${input}Preview(){
		...
		for (var i=0; i<urls.length; i++){
			if (urls[i]!=""){//<c:if test="${type eq 'thumb' || type eq 'images'}">
				li = "<li class=\"li-horizontal-image\"><a href=\""+urls[i]+"\" target=\"_blank\"><img src=\""+urls[i]+"\" url=\""+urls[i]+"\" style=\"max-width:${empty maxWidth ? 100 : maxWidth}px;max-height:${empty maxHeight ? 100 : maxHeight}px;_height:${empty maxHeight ? 200 : maxHeight}px;border:0;padding:3px;\"></a>";//</c:if><c:if test="${type ne 'thumb' && type ne 'images'}">
				li = "<li class=\"li-horizontal-file\"><a href=\""+urls[i]+"\" url=\""+urls[i]+"\" target=\"_blank\">"+decodeURIComponent(urls[i].substring(urls[i].lastIndexOf("/")+1))+"</a>";//</c:if>
