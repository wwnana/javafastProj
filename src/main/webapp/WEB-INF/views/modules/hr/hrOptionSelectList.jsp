	...
	<div class="wrapper-content">
		<div class="ibox">
			...
			<sys:message content="${message}"/>
				...
				<div class="row">
					<div class="col-sm-12">
						<form:form id="searchForm" modelAttribute="hrOption" action="${ctx}/hr/hrOption/selectList" method="post" class="form-inline">
				...
				<table id="contentTable" class="table table-bordered table-striped table-hover">
					...
					<c:forEach items="${page.list}" var="hrOption">
						<tr>
							...
							<td class="codelabel"><fmt:formatDate value="${hrOption.grantDate}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
							<td>${hrOption.grantNum}</td>
							<td>${hrOption.proportion}</td>
							<td>${hrOption.roundNum}</td>
							<td>${hrOption.lockPeriod}</td>
							<td>${hrOption.matureNum}</td>
							<td>${hrOption.optionFile}</td>
							<td>
								<shiro:hasPermission name="hr:hrOption:view">
									<a href="#" onclick="openDialogView('查看期权', '${ctx}/hr/hrOption/view?id=${hrOption.id}','800px', '500px')" class="btn btn-info btn-xs" title="查看"><i class="fa fa-search-plus"></i> 查看</a>
								</shiro:hasPermission>
							</td>
						</tr>
					</c:forEach>
