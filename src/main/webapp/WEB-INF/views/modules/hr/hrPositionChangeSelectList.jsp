	...
	<div class="wrapper-content">
		<div class="ibox">
			...
			<sys:message content="${message}"/>
				...
				<div class="row">
					<div class="col-sm-12">
						<form:form id="searchForm" modelAttribute="hrPositionChange" action="${ctx}/hr/hrPositionChange/selectList" method="post" class="form-inline">
							...
							<table:searchRow></table:searchRow><!-- 查询栏隐藏 -->
								...
								<div class="form-group"><span>员工ID：</span>
				...
				<table id="contentTable" class="table table-bordered table-striped table-hover">
					...
					<c:forEach items="${page.list}" var="hrPositionChange">
						<tr>
							...
							<td class="codelabel">${hrPositionChange.oldOffice.name}</td>
							<td>${hrPositionChange.oldPosition}</td>
							<td>${hrPositionChange.oldPositionLevel}</td>
							<td>${hrPositionChange.office.name}</td>
							<td>${hrPositionChange.position}</td>
							<td>${hrPositionChange.positionLevel}</td>
							<td><fmt:formatDate value="${hrPositionChange.changeDate}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
							<td>
								<shiro:hasPermission name="hr:hrPositionChange:view">
									<a href="#" onclick="openDialogView('查看调岗', '${ctx}/hr/hrPositionChange/view?id=${hrPositionChange.id}','800px', '500px')" class="btn btn-info btn-xs" title="查看"><i class="fa fa-search-plus"></i> 查看</a>
								</shiro:hasPermission>
							</td>
						</tr>
					</c:forEach>
