...
<div class="">
		<div class="">
			...
			<sys:message content="${message}"/>
				...
				<!-- 查询条件 -->
				<c:if test="${empty crmChance.customer.id}">
				<div class="row">
					<div class="col-sm-12">
						<form:form id="searchForm" modelAttribute="crmChance" action="${ctx}/crm/crmChance/selectList" method="post" class="form-inline">
							...
							<table:sortColumn id="orderBy" name="orderBy" value="${page.orderBy}" callback="sortOrRefresh();"/><!-- 支持排序 -->
								...
								<div class="form-group"><span>销售阶段：</span>
									<form:select path="periodType" class="form-control input-small">
								...
								<div class="form-group"><span>商机类型：</span>
									<form:select path="changeType" class="form-control input-small">
										<form:option value="" label=""/>
								...
								<div class="form-group"><span>下次联系时间：</span>
									<input name="beginNextcontactDate" type="text" readonly="readonly" maxlength="20" class="laydate-icon form-control input-small"
										value="<fmt:formatDate value="${crmChance.beginNextcontactDate}" pattern="yyyy-MM-dd"/>"
										onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});"/> - 
									<input name="endNextcontactDate" type="text" readonly="readonly" maxlength="20" class="laydate-icon form-control input-small"
										value="<fmt:formatDate value="${crmChance.endNextcontactDate}" pattern="yyyy-MM-dd"/>"
										onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});"/>
								</div>
								...
								<div class="form-group"><span>创建者：</span>
									<sys:treeselect id="createBy" name="createBy.id" value="${crmChance.createBy.id}" labelName="createBy.name" labelValue="${crmChance.createBy.name}"
										title="用户" url="/sys/office/treeData?type=3" cssClass="form-control input-small" allowClear="true" notAllowSelectParent="true"/>
								</div>
								<div class="form-group"><span>创建时间：</span>
									<input name="beginCreateDate" type="text" readonly="readonly" maxlength="20" class="laydate-icon form-control input-small"
										value="<fmt:formatDate value="${crmChance.beginCreateDate}" pattern="yyyy-MM-dd"/>"
										onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});"/> - 
									<input name="endCreateDate" type="text" readonly="readonly" maxlength="20" class="laydate-icon form-control input-small"
										value="<fmt:formatDate value="${crmChance.endCreateDate}" pattern="yyyy-MM-dd"/>"
										onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});"/>
								</div>
								--%>
				...
				<table id="contentTable" class="table table-bordered table-striped table-hover">
					<thead>
						<tr>
							...
							<th>商机名称</th>
							<th>客户</th>
							<th class="sort-column sale_amount">销售金额</th>
							<th class="sort-column period_type">销售阶段</th>
							<th class="sort-column probability">赢单率</th>
					...
					<c:forEach items="${page.list}" var="crmChance">
						<tr>
							...
							<td class="codelabel">${fns:abbr(crmChance.name,50)}</td>
							<td>${fns:abbr(crmChance.customer.name,50)}</td>
							<td>${crmChance.saleAmount}</td>
							<td>${fns:getDictLabel(crmChance.periodType, 'period_type', '')}</td>
							<td>${fns:getDictLabel(crmChance.probability, 'probability_type', '')}</td>
							<td>${fns:getDictLabel(crmChance.changeType, 'change_type', '')}</td>
							<td>${fns:getDictLabel(crmChance.sourType, 'sour_type', '')}</td>
							
							<td>
								<shiro:hasPermission name="crm:crmChance:view">
									<a href="#" onclick="openDialogView('查看商机', '${ctx}/crm/crmChance/index?id=${crmChance.id}','800px', '500px')" class="btn btn-info btn-xs" title="查看"><i class="fa fa-search-plus"></i> 查看</a>
								</shiro:hasPermission>
							</td>
						</tr>
					</c:forEach>
