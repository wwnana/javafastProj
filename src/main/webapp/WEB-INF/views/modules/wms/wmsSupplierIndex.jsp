...
<head>
	...
	<script type="text/javascript">
		function refresh(){//刷新			
			window.location="${ctx}/wms/wmsSupplier/index";
		}
	</script>
</head>
<body class="">
	<div class="">
		<div class="row">
        	<div class="col-sm-2" style="padding-right: 0;border-right: 1px solid #dee5e7;">
        		<div class="">
                    <div class="dashboard-header gray-bg" style="height: 45px;">                         
                        <h5 onclick="refresh()">供应商分类</h5>
                        <div class="pull-right">
                        <shiro:hasPermission name="wms:wmsProduct:edit">
                            <a href="${ctx}/wms/wmsSupplierType/" class="btn btn-white btn-sm">分类维护</a>
                    	</shiro:hasPermission>
                    	</div>
                    </div>
        	...
        	<div class="col-sm-10" id="right"  style="padding-left:0;">
        		<iframe id="listContent" name="listContent" src="${ctx}/wms/wmsSupplier/list" width="100%" height="100%" frameborder="0"></iframe>
        	</div>
        </div>
	</div>
	<script type="text/javascript">
		var setting = {data:{simpleData:{enable:true,idKey:"id",pIdKey:"pId",rootPId:'0'}},
			callback:{onClick:function(event, treeId, treeNode){
					...
					$('#listContent').attr("src","${ctx}/wms/wmsSupplier/list?productType.id="+id+"&productType.name="+treeNode.name);
				}
			}
		};
		
		function refreshTree(){
			$.getJSON("${ctx}/wms/wmsSupplierType/treeData",function(data){
