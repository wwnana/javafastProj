	...
	<div class="wrapper-content">
		<div class="ibox">
			...
			<sys:message content="${message}"/>
				...
				<div class="row">
					<div class="col-sm-12">
						<form:form id="searchForm" modelAttribute="scmComplaint" action="${ctx}/scm/scmComplaint/selectList" method="post" class="form-inline">
							...
							<table:sortColumn id="orderBy" name="orderBy" value="${page.orderBy}" callback="sortOrRefresh();"/><!-- 支持排序 -->
								...
								<div class="form-group"><span>订单合同：</span>
									<form:input path="omContract.id" htmlEscape="false" maxlength="30" class="form-control input-medium"/>
								</div>
								...
								<div class="form-group"><span>截止日期：</span>
									<input name="beginEndDate" type="text" readonly="readonly" maxlength="20" class="laydate-icon form-control input-medium"
										value="<fmt:formatDate value="${scmComplaint.beginEndDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
										onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/> - 
									<input name="endEndDate" type="text" readonly="readonly" maxlength="20" class="laydate-icon form-control input-medium"
										value="<fmt:formatDate value="${scmComplaint.endEndDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
										onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/>
								</div>
								<div class="form-group"><span>处理状态：</span>
									<form:input path="status" htmlEscape="false" maxlength="1" class="form-control input-medium"/>
								</div>
				...
				<table id="contentTable" class="table table-striped table-bordered table-hover table-condensed">
					...
					<c:forEach items="${page.list}" var="scmComplaint">
						<tr>
							...
							<td class="codelabel">${scmComplaint.name}</td>
							<td>${scmComplaint.order.id}</td>
							<td>${scmComplaint.customer.id}</td>
							<td>${scmComplaint.ownBy.id}</td>
							<td>${fns:getDictLabel(scmComplaint.levelType, 'level_type', '')}</td>
							<td><fmt:formatDate value="${scmComplaint.endDate}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
							<td>${scmComplaint.status}</td>
							<td><fmt:formatDate value="${scmComplaint.dealDate}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
							<td>${fns:getDictLabel(scmComplaint.satisfyType, 'satisfy_type', '')}</td>
							<td>${scmComplaint.auditBy.id}</td>
							<td><fmt:formatDate value="${scmComplaint.auditDate}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
							<td>
								<shiro:hasPermission name="scm:scmComplaint:view">
									<a href="#" onclick="openDialogView('查看客户投诉', '${ctx}/scm/scmComplaint/view?id=${scmComplaint.id}','800px', '500px')" class="btn btn-info btn-xs" title="查看"><i class="fa fa-search-plus"></i> 查看</a>
								</shiro:hasPermission>
							</td>
						</tr>
					</c:forEach>
