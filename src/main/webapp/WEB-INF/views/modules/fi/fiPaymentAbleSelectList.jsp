...
<div class="wrapper-content">
		<div class="ibox">
			...
			<sys:message content="${message}"/>
				...
				<div class="row">
					<div class="col-sm-12">
						<form:form id="searchForm" modelAttribute="fiPaymentAble" action="${ctx}/fi/fiPaymentAble/selectList" method="post" class="form-inline">
							...
							title="采购单" url="${ctx}/wms/wmsPurchase/selectList" cssClass="form-control input-medium" dataMsgRequired=""  allowClear="false" allowInput="false"/>
								</div>
								<div class="form-group"><span>供应商：</span>
									<sys:tableselect id="supplier" name="supplier.id" value="${fiPaymentAble.supplier.id}" labelName="supplier.name" labelValue="${fiPaymentAble.supplier.name}" 
										title="供应商" url="${ctx}/wms/wmsSupplier/selectList" cssClass="form-control input-medium" dataMsgRequired=""  allowClear="false" allowInput="false"/>
								</div>
								<div class="form-group"><span>应付时间：</span>
									<input name="beginAbleDate" type="text" readonly="readonly" maxlength="20" class="laydate-icon form-control input-medium"
										value="<fmt:formatDate value="${fiPaymentAble.beginAbleDate}" pattern="yyyy-MM-dd"/>"
										onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});"/> - 
									<input name="endAbleDate" type="text" readonly="readonly" maxlength="20" class="laydate-icon form-control input-medium"
										value="<fmt:formatDate value="${fiPaymentAble.endAbleDate}" pattern="yyyy-MM-dd"/>"
										onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});"/>
								</div>
				...
				<table id="contentTable" class="table table-bordered table-striped table-hover">
					<thead>
						<tr>
							...
							<th class="sort-column p.no">采购单</th>
							<th class="sort-column s.name">供应商</th>
							<th class="sort-column a.amount">应付金额</th>
							<th class="sort-column a.real_amt">实际已付</th>
							<th class="sort-column a.able_date">应付时间</th>
					...
					<c:forEach items="${page.list}" var="fiPaymentAble">
						<tr>
							...
							<td class="codelabel">${fiPaymentAble.no}</td>
							<td>${fiPaymentAble.purchase.no}</td>
							<td>${fiPaymentAble.supplier.name}</td>
							...
							<td>${fiPaymentAble.ownBy.name}</td>
							<td>${fns:getDictLabel(fiPaymentAble.status, 'finish_status', '')}</td>
							<%-- 
							<td>${fiPaymentAble.createBy.name}</td>
							<td><fmt:formatDate value="${fiPaymentAble.createDate}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
							--%>
							<td>
								<shiro:hasPermission name="fi:fiPaymentAble:view">
									<a href="#" onclick="openDialogView('查看应付款', '${ctx}/fi/fiPaymentAble/index?id=${fiPaymentAble.id}','800px', '500px')" class="btn btn-info btn-xs" title="查看"><i class="fa fa-search-plus"></i> 查看</a>
								</shiro:hasPermission>
							</td>
						</tr>
					</c:forEach>
