								...
								<tr>
									<td>
										...
										<c:if test="${not empty task.assignee}">
											<a href="${ctx}/act/task/form?taskId=${task.id}&taskName=${fns:urlEncode(task.name)}&taskDefKey=${task.taskDefinitionKey}&procInsId=${task.processInstanceId}&procDefId=${task.processDefinitionId}&status=${status}&id=${project.id}">${fns:abbr(not empty vars.map.title ? vars.map.title : task.id, 60)}</a>
										</c:if>
									</td>
									<td>
									...
									<td>
										...
										<c:if test="${not empty task.assignee}"><%--
											...
											<a href="${ctx}/act/task/form?taskId=${task.id}&taskName=${fns:urlEncode(task.name)}&taskDefKey=${task.taskDefinitionKey}&procInsId=${task.processInstanceId}&procDefId=${task.processDefinitionId}&status=${status}&id=${project.id}">任务办理</a>
										</c:if>
										...
										<a target="_blank" href="${ctx}/act/task/processMap/${task.processDefinitionId}/${task.processInstanceId}">跟踪信息</a>
									</td>
								</tr>
							</c:forEach>
						</tbody>
