	...
	<div class="weui-panel weui-panel_access">
     ...
     <div class="weui-tabbar">
		...
		<c:if test="${fns:getUser().id == crmService.ownBy.id && crmService.status != 2}">
			<a href="${ctx}/mobile/crm/crmService/deal?id=${crmService.id}" onclick="return confirmx('确认要工单已经完成了吗？', this.href)" class="weui-tabbar__item weui-navbar__item"> 完成</a>
		</c:if>
		<c:if test="${crmService.status == 2 && crmService.auditStatus != 1}">
  			<a href="${ctx}/mobile/crm/crmService/auditForm?id=${crmService.id}" class="weui-tabbar__item weui-navbar__item"> 审核</a>
		</c:if>
	 </div>
</div>
</body>
