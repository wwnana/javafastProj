...
layui.use('layim', function(layim){
	...
	// 连接成功建立的回调方法
	  webSocket.onopen = function(event) {
		  var arr=new Object();
		  arr.subtype="_online_user_";
		  arr.userId=currentId;
	...
	// 接收到消息的回调方法
	  webSocket.onmessage = function(event) {
	    var res=event.data;
	    var dataJson = jQuery.parseJSON(res);
		if (dataJson.success) {
			var dataBody = dataJson.body;
			
			if (dataBody.subtype == "OA_NOTIFY_MSG") {
				top.layer.open({
					type : 2,
				 	shade:0,
					offset : 'rb',
					...
					content : url+"/oa/oaNotify/notRead"
				});
			}else if (dataBody.subtype == "OA_MAIL_MSG") {
				...
				top.layer.open({
					...
					title : "消息提醒",
					maxmin : false, //开启最大化最小化按钮
					content : url+"/iim/mailBox/notRead"
				});
			}else if( dataBody.subtype == "_online_all_status_"){
				setTimeout(function(){updateOnlineStatus(dataBody.users);},500);
			}else if( dataBody.subtype == "_sys_"){
				var fromId=dataBody.fromId;
		    	var contents=dataBody.contents;
	            var obj = {
	                username: "系统通知"
	            ...
	            return;
			}else if(dataBody.subtype == "_msg_"){
				debugger;
				var fromId=dataBody.fromUser;
		    	var toId=dataBody.toUser;
		    	var content=dataBody.content;
		    	var avatar=dataBody.avatar;
		    	var type=dataBody.type;
		    	var fromUserName=dataBody.fromUserName;
		    	var sendtime = dataBody.datatime;
	  ...
	  function send(mine,To) {
		  var arr=new Object();
		  arr.subtype="_msg_";
		  arr.fromUser=currentId;
		  arr.toUser=To.id;
		  arr.content=mine.content;
		  arr.avatar=mine.avatar;
		  arr.type=To.type;
		  arr.fromUsername=currentName;
		  arr.datatime="NAN";
		  var  msg=JSON.stringify(arr);
		  webSocket.send(msg);
