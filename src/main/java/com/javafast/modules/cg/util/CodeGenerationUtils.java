package com.javafast.modules.cg.util;
...
public class CodeGenerationUtils {
	...
	private static Logger logger = LoggerFactory.getLogger(CodeGenerationUtils.class);

	/**
	 * 初始化列属性字段
	 * @param cgTable
	 */
	public static void initColumnField(CgTable cgTable){
		for (CgTableColumn column : cgTable.getColumnList()){
			...
			// 是否是主键
			column.setIsPk(cgTable.getPkList().contains(column.getName())?"1":"0");
			...
			// 创建者、更新者
				...
				column.setJavaType(User.class.getName());
				column.setJavaField(column.getJavaField() + ".id|name");
			}
			// 创建时间、更新时间
	...
	public static String getTemplatePath(){
		try{
			...
			if(file != null){
				return file.getAbsolutePath() + File.separator + StringUtils.replaceEach(CodeGenerationUtils.class.getName(), 
						new String[]{"util."+CodeGenerationUtils.class.getSimpleName(), "."}, new String[]{"template", File.separator});
			}			
		}catch(Exception e){
			logger.error("{}", e);
		}
...
//		}
	 ...
	 * @return
	 */
	public static CgConfig getConfig(){
		return fileToObject("config.xml", CgConfig.class);
	}
	 ...
	 * @return
	 */
	public static List<CgTemplate> getTemplateList(CgConfig config, String category, boolean isChildTable){
		List<CgTemplate> templateList = Lists.newArrayList();
		if (config !=null && config.getCategoryList() != null && category !=  null){
			for (CgCategory e : config.getCategoryList()){
				if (category.equals(e.getValue())){
					...
					if (list != null){
						for (String s : list){
							if (StringUtils.startsWith(s, CgCategory.CATEGORY_REF)){
								templateList.addAll(getTemplateList(config, StringUtils.replace(s, CgCategory.CATEGORY_REF, ""), false));
							}else{
								CgTemplate template = fileToObject(s, CgTemplate.class);
								if (template != null){
	 ...
	 * @return
	 */
	public static Map<String, Object> getDataModel(CgTable cgTable){
		Map<String, Object> model = Maps.newHashMap();
		
		model.put("packageName", StringUtils.lowerCase(cgTable.getPackageName()));
		model.put("lastPackageName", StringUtils.substringAfterLast((String)model.get("packageName"),"."));
		model.put("moduleName", StringUtils.lowerCase(cgTable.getModuleName()));
		model.put("subModuleName", StringUtils.lowerCase(cgTable.getSubModuleName()));
		model.put("className", StringUtils.uncapitalize(cgTable.getClassName()));
		model.put("ClassName", StringUtils.capitalize(cgTable.getClassName()));
		
		model.put("functionName", cgTable.getFunctionName());
		model.put("functionNameSimple", cgTable.getFunctionNameSimple());
		model.put("functionAuthor", StringUtils.isNotBlank(cgTable.getFunctionAuthor())?cgTable.getFunctionAuthor():UserUtils.getUser().getName());
		model.put("functionVersion", DateUtils.getDate());
		model.put("pageModel", cgTable.getPageModel());
		model.put("treeData", cgTable.getTreeData());
		
		model.put("urlPrefix", model.get("moduleName")+(StringUtils.isNotBlank(cgTable.getSubModuleName())
				?"/"+StringUtils.lowerCase(cgTable.getSubModuleName()):"")+"/"+model.get("className"));
		model.put("viewPrefix", //StringUtils.substringAfterLast(model.get("packageName"),".")+"/"+
				model.get("urlPrefix"));
		model.put("permissionPrefix", model.get("moduleName")+(StringUtils.isNotBlank(cgTable.getSubModuleName())
				?":"+StringUtils.lowerCase(cgTable.getSubModuleName()):"")+":"+model.get("className"));
		
		model.put("dbType", Global.getConfig("jdbc.type"));

		model.put("table", cgTable);
	 ...
	 * @return
	 */
	public static String generateToFile(CgTemplate tpl, Map<String, Object> model, boolean isReplaceFile){
		// 获取生成文件
