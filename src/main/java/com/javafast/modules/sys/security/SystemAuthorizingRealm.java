...
import com.javafast.common.web.Servlets;
...
import com.javafast.modules.main.web.LoginController;
...
public class SystemAuthorizingRealm extends AuthorizingRealm {
	...
	public static final String SYSTEM_DEFAULT_LOGIN = "0";
	public static final String MOBILE_CODE_LOGIN = "1";
	public static final String WEHART_AUTH_LOGIN = "2";
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) {
		...
		//登录类型：0：账号密码登录
		if(SYSTEM_DEFAULT_LOGIN.equals(token.getLoginType())){
		...
		//登录类型：1：手机验证码登录，2：企业微信授权登录
		if(MOBILE_CODE_LOGIN.equals(token.getLoginType()) || WEHART_AUTH_LOGIN.equals(token.getLoginType())){
			
			//下次自动登录
			token.setRememberMe(true);
			
			//免密码登录
			user.setPassword(UserService.entryptPassword(user.getLoginName()));			
			token.setPassword(user.getLoginName().toCharArray());
			byte[] salt = Encodes.decodeHex(user.getPassword().substring(0,16));			
			return new SimpleAuthenticationInfo(new Principal(user, token.isMobileLogin()), 
					user.getPassword().substring(16), ByteSource.Util.bytes(salt), getName());
		}
